...
async function createkey(event){
    ...
    async function connectDropbox(){
        setCode('dropbox')
        window.open('https://www.dropbox.com/oauth2/authorize?client_id=rqiucchpvi1uywj&redirect_uri=http://localhost:3000/meruapp&token_access_type=offline&response_type=code&state=dropbox')
    }
            ...
            try{
                ...
                if(state != ''){
                    if(state == 'dropbox'){
                        ...
                        dbx.auth.getAccessTokenFromCode('http://localhost:3000/meruapp', authcode).then((token) => {
                    console.log('hi!')
